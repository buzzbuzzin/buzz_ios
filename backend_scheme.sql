...
CREATE TABLE public.badges (
  ...
  badge_type text DEFAULT 'course'::text CHECK (badge_type = ANY (ARRAY['course'::text, 'ex_military'::text, 'buzz'::text, 'government_employee'::text, 'faa'::text, 'flight_reviewer'::text, 'roc_a_examiner'::text, 'beacon_volunteer'::text, 'cert'::text])),
...
CREATE TABLE public.badges_catalog (
  id uuid NOT NULL DEFAULT uuid_generate_v4(),
  badge_type text NOT NULL CHECK (badge_type = ANY (ARRAY['course'::text, 'ex_military'::text, 'buzz'::text, 'government_employee'::text, 'faa'::text, 'flight_reviewer'::text, 'roc_a_examiner'::text, 'beacon_volunteer'::text, 'cert'::text])),
  title text NOT NULL,
  category text,
  course_id uuid,
