...
class TransponderService: ObservableObject {
    ...
    func fetchAllActiveTransponders() async throws -> [Transponder] {
        ...
        do {
            ...
            let allTransponders: [Transponder] = try await supabase
                .from("transponders")
                .select()
            ...
            // Filter out transponders with null location data
            let transponders = allTransponders.filter { transponder in
                transponder.lastLocationLat != nil && transponder.lastLocationLng != nil
            }
    ...
    func updateTransponderLocation(
        ...
        // Check if demo mode is enabled
        if DemoModeManager.shared.isDemoModeEnabled {
            // Update local array instead of backend
            await MainActor.run {
    ...
    func deleteTransponder(transponderId: UUID, pilotId: UUID) async throws {
        ...
        if DemoModeManager.shared.isDemoModeEnabled {
            ...
            // Remove from local array instead of backend
            await MainActor.run {
